...
class AccountMove(models.Model):
    ...
    def _ubl_add_header(self, parent_node, ns, version="2.1"):
        ...
        doc_id.text = self.system_inv_no
        ...
        # new_form_date=''
        # i=0
        # for year in self.invoice_date_time.split('/'):
        #     # if i == 0:
        #     #     i+=1
            #     new_form_date = year
            # else:
        new_form_date = self.invoice_date_time.split('/')[2]+'-'+self.invoice_date_time.split('/')[1]+'-'+self.invoice_date_time.split('/')[0]
        issue_date.text = new_form_date
    ...
    def new_additionalreference(self,parent_node, ns, version="2.1"):
        ...
        attach_node.text = str(self.uuid)
    ...
    def billing_refence(self,parent_node, ns, version="2.1"):
        ...
        ctx["no_embedded_ubl_xml"] = True
        ctx["force_report_rendering"] = True
        binary_node.text = "Invoice Number :"+str(self.system_inv_no)+" ;"+"Invoice Issue Date :"+str(self.invoice_date)
    ...
    #     # ):
    #     # self.signature_refence(parent_node, ns, version="2.1")
    #     filename = "Invoice-" + self.system_inv_no + ".pdf"
    #     docu_reference = etree.SubElement(
    #         parent_node, ns["cac"] + "AdditionalDocumentReference"
    #     )
    #     docu_reference_id = etree.SubElement(docu_reference, ns["cbc"] + "ID")
    ...
    def _ubl_add_legal_monetary_total(self, parent_node, ns, version="2.1"):
        ...
        cur_name = self.curr_code or None
        ...
        tax_incl_total.text = "%0.*f" % (prec, self.net_amount_with_comma)
        ...
        prepaid_value = self.advance_comma
        ...
        payable_amount.text = "%0.*f" % (prec, self.net_amount_with_comma)
    ...
    def _ubl_add_tax_total(self, xml_root, ns, version="2.1"):
        ...
        tax_amount_node.text = "%0.*f" % (prec, self.vat_amount_comma)
        # if not float_is_zero(self.amount_tax, precision_digits=prec):
        if not float_is_zero(self.vat_amount_comma, precision_digits=prec):
    ...
    def generate_invoice_ubl_xml_etree(self, version="2.1"):
        ...
        tax_amount_nodes.text = "%0.*f" % (prec, self.vat_amount_comma)
